...
только `wiki/`, её не сдвигают. Перед доработкой кода проверять, менялся ли код
после `commit`: `git log --oneline <commit>..HEAD -- hunt web tests scripts`.
Если есть коммиты — перечитать исходники и обновить страницу, подняв
