...
`commit` во frontmatter — ревизия **кода** (последний коммит, менявший `hunt/`,
`web/`, `tests/`, скрипты), к которой синхронизирован текст; коммиты, менявшие
только `wiki/`, её не сдвигают. Перед доработкой кода сверять `commit`
затрагиваемых страниц с `git log -1 --format=%h -- hunt web tests scripts`.
Если код с тех пор менялся — перечитать исходники и обновить страницу, подняв
`updated`/`commit`. После значимого изменения кода обновлять затронутые страницы
и дописывать запись в `log.md` (ingest).
