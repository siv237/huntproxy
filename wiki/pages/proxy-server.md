...
### Апстрим бейджа и история переключений

Бейдж (пинг, миниграфик, адрес/гео) относится **только к пулу** и не зависит от
маршрутизации по доменам: `ProxyRunner.current_pool_upstream()`
(`hunt/proxy_runner.py`) возвращает жёстко выбранную прокси (`select`), иначе
прокси, которая последней реально отвезла пул-трафик (`_effective_upstream`,
TTL 300 c), иначе — пусто, и бейдж скрыт. Переход по отказу отдаёт прокси, на
которую перескочил пул, как `fallback`.

`_effective_upstream` обновляется только по факту обслуженного запроса — из
фаннела логирования трафика `_queue_traffic_log` (`hunt/db.py`), статус `ok`.
Пинг (`_ping_source`, `hunt/proxy_ping.py`) пингует этот же прокси; в бейдже
пометки «авто»/«фолбэк».

История (`record_upstream_switch`) ведётся по изменениям этого пулового
апстрима из тика пинга: только смена **адреса**, одна строка на прокси, без
маршрутных изменений и повторов. Старые записи багованного формата разово
вычищены миграцией `switch_hist_v2` (`hunt/state_persistence.py`).
