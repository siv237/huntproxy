...
## [2026-09-22] ingest | Перезапуск с включённым перехватом

- Дефект: правила перехвата переживают рестарт, а cgroup самоисключения хранила
  старый PID → новый процесс прокси перехватывал сам себя (петля, 100% CPU,
  пропадал интернет).
- `reenforce_on_startup()` (из `main.py`): при включённом режиме переприменяет
  правила с текущим PID; при ошибке или без прозрачного прокси снимает правила,
  чтобы не осталась «чёрная дыра».
...
## [2026-09-26] ingest | История переключений: тип маршрута — не смена прокси

- `record_effective_upstream` (`hunt/switch_history.py`) писал запись при смене
  пары (адрес+тип), поэтому один и тот же прокси, обслуживающий и дефолт
  (`pool_selected` → «выбрано»), и пул-список (ютуб → «авто»), давал по две
  записи на каждое переключение маршрута по домену и дробил периоды трафика
  (в проде — сотни строк по одному адресу `185.87.255.47:1080`).
- Теперь история пишется только при смене **адреса**; смена типа обновляет
  `_effective_upstream` и тип последней записи. Direct — маршрут, не прокси:
  состояние обновляется, запись не создаётся. `_merge_consecutive` схлопывает
  соседние записи по адресу (тип — метаданные), `_build_switch_history`
  отбрасывает записи без прокси, поэтому старый шум тоже схлопывается.
- Страница: [proxy-server](pages/proxy-server.md).

## [2026-09-26] ingest | История смен: только смены текущего апстрима

- В хронологию пишутся только смены **дефолтного** апстрима: `_connect_upstream`
  различает маршрут из списка и дефолтный (`_resolve_route_matched`,
  `hunt/routing.py`), и для list-matched трафика обновляет только
  `_effective_upstream` (бейдж), без записи в историю.
- Запись создаётся при смене адреса; смена типа на том же прокси — метаданные;
  `direct` не пишется. Разово вычищены записи старого багованного формата
  (миграция `switch_hist_v2`), поэтому список начинается с чистого листа и
  показывает только реальные смены прокси.

## [2026-09-26] ingest | Бейдж = фактически обслуженный прокси

- `_effective_upstream` (источник бейджа в шапке) обновляется из фаннела
  логирования трафика `_queue_traffic_log` (`hunt/db.py`) по последнему
  токену цепочки, только для записей со статусом `ok`. Раньше значение
  выставлялось в `_connect_upstream` в момент попытки соединения — бейдж мог
  показывать прокси, который ничего не отдал (на проде: бейдж `24.52.147.103`
  при трафике через `185.87.255.47`), и дёргался от доменных маршрутов.
- История (`record_upstream_switch`) пишет только смены адреса текущего
  (дефолтного) апстрима; list-matched трафик записей не создаёт.
- Страница: [proxy-server](pages/proxy-server.md).

## [2026-09-26] ingest | Бейдж и история — только про пул

- `ProxyRunner.current_pool_upstream()`: жёсткий выбор → `select`, иначе
  последний прокси, реально отвёзший пул-трафик (TTL 300 c), иначе пусто →
  бейдж скрыт; фолбэк → `fallback`. Маршрутизация по доменам не учитывается.
- `_effective_upstream` пишется из фаннела трафика (`_queue_traffic_log`) по
  ok-записям; история — из тика пинга по смене адреса (`record_upstream_switch`).
- Страницы: [proxy-server](pages/proxy-server.md), [frontend](pages/frontend.md).
