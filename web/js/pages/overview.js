router.register('overview', (container) => {
  ...
  function buildQuickActionsCard() {
    ...
    const actions = [
      ...
      { icon: 'upload', label: t('page.overview.import'), desc: t('page.overview.importDesc'), action: () => { const inp = document.createElement('input'); inp.type = 'file'; inp.accept = '.txt'; inp.onchange = () => { const f = inp.files[0]; if (!f) return; const reader = new FileReader(); reader.onload = () => { const lines = reader.result.split('\n').map(l => l.trim()).filter(l => l); api.importProxies({ proxies: lines }).then(r => { app.toast(t('page.overview.import') + ': ' + (r.added || lines.length)); }).catch(e => app.toast(t('common.error', {message: e.message}), 'error')); }; reader.readAsText(f); }; inp.click(); }, color: 'var(--warning)', bg: 'rgba(245,158,11,0.1)' },
