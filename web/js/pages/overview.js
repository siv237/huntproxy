router.register('overview', (container) => {
  ...
  function buildStatsCard() {
    ...
    card.id = 'stats-card';
    card.appendChild(ui.el('div', 'card-title', { text: 'Pool Stats', style: 'margin-bottom:8px' }));
    const grid = ui.el('div', '', { style: 'display:grid;grid-template-columns:1fr 1fr;gap:8px' });
    grid.appendChild(statBox('Alive', '0', 'stat-alive', 'var(--success)'));
    grid.appendChild(statBox('Dead', '0', 'stat-dead', 'var(--danger)'));
    grid.appendChild(statBox('Blacklist', '0', 'stat-bl', 'var(--warning)'));
  ...
  function statBox(label, value, id, color) {
    ...
    box.appendChild(ui.el('div', '', { id, style: `font-size:20px;font-weight:700;color:${color}`, text: value }));
  ...
  function buildProgressCard() {
    ...
    card.appendChild(ui.el('div', 'card-title', { text: 'Progress', style: 'margin-bottom:8px' }));
  ...
  function buildBlacklistCard() {
    ...
    addBtn.addEventListener('click', () => {
      ...
      if (!a) return;
      api.blAdd(a, r).then(() => { addrInp.value = ''; reasonInp.value = ''; app.toast('Added to blacklist'); load(); });
    });
  ...
  function buildLogCard() {
    ...
    const log = ui.el('div', '', { id: 'hunt-log', style: 'font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:11px;line-height:1.5;overflow-y:auto;color:var(--text-primary)' });
