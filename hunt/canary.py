...
class CanaryMixin:
    ...
    async def _check_canary(self) -> dict:
            ...
            results, latencies = await self._canary_probe_hosts(hosts)
            ...
            self._emit_canary_status(was_alive, alive)
            direct = await self._canary_direct_info(alive)
            self._canary_record(alive, alive_count, total, results, direct)
            return {
                "alive": alive, "hosts": results, "latencies": latencies,
                "alive_count": alive_count, "total": total, **direct,
            }

    async def _canary_probe_hosts(self, hosts) -> tuple:
        results, latencies = {}, {}
    ...
    def _emit_canary_status(self, was_alive, alive):
    ...
    async def _canary_direct_info(self, alive) -> dict:
        if not alive:
            return {"direct_ip": "", "direct_country": "", "direct_isp": "", "direct_city": ""}
        try:
            reader, writer = await self._outbound_connect("ip-api.com", 80, timeout=25 if self._channel_is_set() else 8)
            ...
            old_ip = getattr(self, '_canary_last_ip', '')
            if old_ip and new_ip and old_ip != new_ip:
                self._emit(f"ISP changed: {old_ip} ({getattr(self, '_canary_last_isp', '')}) → {new_ip} ({data.get('isp', '')})", "warn")
            self._canary_last_ip = new_ip
            self._canary_last_isp = data.get("isp", "")
            self._canary_last_country = data.get("country", "")
            self._canary_last_city = data.get("city", "")
            return {
                "direct_ip": new_ip, "direct_country": data.get("country", ""),
                "direct_isp": data.get("isp", ""), "direct_city": data.get("city", ""),
            }
        except Exception:
            return {"direct_ip": "", "direct_country": "", "direct_isp": "", "direct_city": ""}

    def _canary_record(self, alive, alive_count, total, results, direct):
        try:
            conn = self._stats_db()
            conn.execute(
                ...
                (time.time(), 1 if alive else 0, alive_count, total,
                 json.dumps(results), direct["direct_ip"], direct["direct_country"],
                 direct["direct_isp"], direct["direct_city"])
            )
            conn.commit()
            conn.close()
        except Exception:
