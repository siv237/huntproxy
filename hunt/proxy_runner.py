...
from hunt.proxy_http import ProxyHttpMixin
from hunt.switch_history import enrich_switch_history, record_switch
import logging
...
class ProxyRunner(ProxyRouteMixin, ProxyHttpMixin):
    ...
    def _record_switch(self, action: str, address: Optional[str]):
        record_switch(self.state._proxy_switch_history, action, address or "")
    ...
    def _log(self, peer, target, status, upstream="", bytes_in=0, bytes_out=0, duration=0.0):
        entry = {"ts": time.time(), "client": peer[0] if peer else "?", "target": target, "status": status, "upstream": upstream, "bytes_in": bytes_in, "bytes_out": bytes_out, "duration": round(duration, 3), "via": "http"}
        ...
        try:
            self.state._queue_traffic_log(
                (entry["ts"], entry["client"], target, status, upstream,
                 bytes_in, bytes_out, round(duration, 3), "http"))
        except Exception:
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "switch_history": enrich_switch_history(self.state),
        }
