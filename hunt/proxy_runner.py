...
class ProxyRunner(ProxyRouteMixin):
    ...
    def select(self, address: Optional[str], record: bool = True):
    ...
    def _record_switch(self, action: str, address: Optional[str]):
        ...
        if len(self.state._proxy_switch_history) > 500:
            self.state._proxy_switch_history = self.state._proxy_switch_history[-500:]
    ...
    def _enrich_switch_history(self) -> list[dict]:
        ...
        address and whose ts falls within that interval.
        """
        hist = self.state._proxy_switch_history[-500:]
        if not hist:
            return []
        now = time.time()
        merged = self._merge_consecutive(hist)
        traffic = self._traffic_by_period(merged, now)
        durations = self._period_durations(merged, now)
    ...
    def _traffic_by_period(self, chronological: list[dict], now: float) -> dict[int, int]:
        ...
        for j, e in enumerate(chronological):
            start = e["ts"]
            end = chronological[j + 1]["ts"] if j + 1 < len(chronological) else now
        ...
        try:
            ...
            for j, (start, end, addr) in intervals.items():
                ...
                row = conn.execute(
                    ...
                    "FROM traffic_log WHERE ts >= ? AND ts < ? AND upstream LIKE ?",
                    (start, end, f"%{addr}%"),
                ).fetchone()
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "switch_history": self._enrich_switch_history(),
        }
